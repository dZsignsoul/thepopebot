...
function loadCrons() {
  ...
  for (const cronEntry of crons) {
    ...
    const task = cron.schedule(schedule, async () => {
      // [cron-harness-bridge] Patch 7: announce cron fires to the harness so
      // /admin/harness/crons + /admin/harness/metrics can see them.
      // Best-effort; harness unreachable does not block cron execution.
      let runId = null;
      const harnessUrl = process.env.HARNESS_URL;
      const harnessKey = process.env.HARNESS_API_KEY;
      if (harnessUrl && harnessKey) {
        try {
          const resp = await fetch(`${harnessUrl.replace(/\/$/, '')}/runs`, {
            method: 'POST',
            headers: { 'x-harness-key': harnessKey, 'content-type': 'application/json' },
            body: JSON.stringify({ kind: 'cron', name, job: cronEntry.job || null }),
          });
          if (resp.ok) {
          ...
          console.error(`[CRON] ${name}: harness announce failed - ${e.message}`);
        }
      }
      try {
        ...
        if (runId && harnessUrl && harnessKey) {
          fetch(`${harnessUrl.replace(/\/$/, '')}/runs/${encodeURIComponent(runId)}`, {
            method: 'PATCH',
            headers: { 'x-harness-key': harnessKey, 'content-type': 'application/json' },
            body: JSON.stringify({ status: 'succeeded' }),
          }).catch(e => console.error(`[CRON] ${name}: harness patch (success) failed - ${e.message}`));
        }
      } catch (err) {
        ...
        if (runId && harnessUrl && harnessKey) {
          fetch(`${harnessUrl.replace(/\/$/, '')}/runs/${encodeURIComponent(runId)}`, {
            method: 'PATCH',
            headers: { 'x-harness-key': harnessKey, 'content-type': 'application/json' },
            body: JSON.stringify({ status: 'failed', error: String(err.message || err).slice(0, 500) }),
          }).catch(e2 => console.error(`[CRON] ${name}: harness patch (failure) failed - ${e2.message}`));
        }
      }
    });
