...
async function launchAgentJobContainer(params) {
  ...
  // Patch 6: explicitly force-remove the container before removing the volume.
  // AutoRemove=true is set on the container, but in practice it races the
  // cleanup here — waitForContainer returns when the container reaches "exited"
  // state, but AutoRemove finishes some milliseconds later. removeVolume can
  // fire first and hit "409 volume in use". removeContainer is idempotent
  // (force=true; 404 is silent), so calling it here is safe whether AutoRemove
