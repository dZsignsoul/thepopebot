 ...
 * @param {string} [options.targetRepo] - "owner/name" per-job repo override
 * @param {string} [options.planSpecPath] - Path under PROJECT_ROOT to a PLAN.md to commit as agent-job/PLAN.md (Patch 13)
 * @param {string} [options.planId] - Parent plan id; surfaced to the container as PLAN_ID env (Patch 13)
...
async function createAgentJob(agentJobDescription, options = {}) {
  ...
  // Patch 13 (task 36b): inject the user's plan spec into the workspace as
  // agent-job/PLAN.md so the planner role can read it on cold-start. Same
  // tree-entry mechanism as Patch 11 — the upstream container clones the
  // branch and finds PLAN.md sitting there. Source path is whitelisted to
  // the EFS plans/ subtree; we reject anything that escapes via .. or starts
  // with / so the harness can't be coaxed into reading arbitrary files.
  // Marker: patch13-plan-spec-injection.
  if (options.planSpecPath) {
    try {
      const rel = String(options.planSpecPath);
      if (rel.includes('..') || path.isAbsolute(rel) || !rel.startsWith('agent-job/plans/')) {
        throw new Error(`plan_spec_path must be a relative path under agent-job/plans/ (got "${rel}")`);
      }
      const planPath = path.join(PROJECT_ROOT, rel);
      const planText = fs.readFileSync(planPath, 'utf8');
      treeEntries.push({
        path: `agent-job/PLAN.md`,
        mode: '100644',
        type: 'blob',
        content: planText,
      });
      console.log(`[agent-job] committing plan spec for plan ${options.planId || '(no id)'} as agent-job/PLAN.md (${planText.length} bytes) — patch13-plan-spec-injection`);
    } catch (e) {
      console.error(`[agent-job] failed to inject plan spec from '${options.planSpecPath}':`, e.message);
      throw e;
    }
  }
