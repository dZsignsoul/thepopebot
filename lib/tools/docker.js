...
async function runAgentJobContainer({ agentJobId, repo, branch, title, description, codingAgent, llmModel }) {
  ...
  // Patch 8: role-specialist system prompt for agent-job runs.
  // If a file exists at agent-job/prompts/SYSTEM_PROMPT.<agent>.md on EFS,
  // load it and inject as SYSTEM_PROMPT env. The codex-cli setup.sh script
  // writes that to AGENTS.md inside the container; Codex picks up AGENTS.md
  // automatically. Best-effort: read errors do not block the spawn.
  ...
  try {
    ...
    if (fs.existsSync(promptPath)) {
      ...
      env.push(`SYSTEM_PROMPT=${promptText}`);
      console.log(`[agent-job] loaded role prompt for agent '${agent}' (${promptText.length} bytes)`);
    }
  } catch (e) {
