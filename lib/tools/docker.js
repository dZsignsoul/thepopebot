...
import { getCustomProvider, getConfigValue } from '../db/config.js';
...
// Resolve coding-agent image with per-agent override from settings DB.
// Override key: CODING_AGENT_${AGENT}_IMAGE. Bypasses getConfig() allowlist
// by querying the underlying table directly.
function resolveAgentImage(agent) {
  const overrideKey = `CODING_AGENT_${agent.toUpperCase().replace(/-/g, '_')}_IMAGE`;
  const override = getConfigValue(overrideKey);
  if (override) return override;
  const version = process.env.THEPOPEBOT_VERSION;
  return `stephengpope/thepopebot:coding-agent-${agent}-${version}`;
}
...
async function runInteractiveContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, injectSecrets, continueSession = true }) {
  ...
  const image = resolveAgentImage(agent);
...
function buildAgentAuthEnv(agent) {
  ...
  // Patch 9: role-agent alias. The codex-cli image is used for harness-routed
  // "role" agents (triage, code, docs, review, …) per Patch 8. Names not in
  // the known set below fall through to the codex-cli auth path so the
  // running container gets OPENAI_API_KEY / CODEX_OAUTH_TOKEN rather than
  // the default anthropic env that buildAgentAuthEnv would otherwise leave
  // empty for unknown names.
  // Marker for [[dockerfile-bundle-lesson]] grep: codex-cli-alias-fallthrough
  const KNOWN_AGENTS = new Set(['claude-code', 'codex-cli', 'gemini-cli', 'pi-coding-agent', 'opencode', 'kimi-cli']);
  if (!KNOWN_AGENTS.has(agent)) {
    console.log(`[buildAgentAuthEnv] aliasing unknown agent '${agent}' to codex-cli auth path (codex-cli-alias-fallthrough)`);
    agent = 'codex-cli';
  }
...
async function runClusterWorkerContainer({ containerName, image, codingAgent, env = [], binds = [], workingDir }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const resolvedImage = image || resolveAgentImage(agent);
...
async function runAgentJobContainer({ agentJobId, repo, branch, title, description, codingAgent, llmModel }) {
  ...
  // Patch 10 (supersedes Patch 8's env approach): prepend role prompt to the
  // job description so the agent reads it as part of its task. Patch 8's
  // SYSTEM_PROMPT env-var approach didn't work — the upstream
  // build-system-prompt.sh script unconditionally blanks SYSTEM_PROMPT in
  // step 1 of setup.sh, then rebuilds from agent-job/SOUL.md +
  // agent-job/SYSTEM.md (which don't exist). Result: SYSTEM_PROMPT arrives
  // empty in AGENTS.md and logs/<id>/system-prompt.md. Verified live
  // 2026-05-21 against branch agent-job/8b667c47.
  //
  // The agent's task prompt is built in 5_build-prompt.sh as:
  //   PROMPT="\n\n# Your Job\n\n${AGENT_JOB_DESCRIPTION}"
  // So we can inject role guidance by prepending it onto description here.
  // Codex sees one combined prompt; the role text leads the request text.
  //
  // Path sanitization (defense-in-depth — harness validates agent on the
  // boundary, but let's not trust callers): only run if agent matches the
  // expected slug shape.
  // Marker for [[dockerfile-bundle-lesson]] grep: SYSTEM_PROMPT.${agent}.md
  let finalDescription = description;
  if (/^[a-z][a-z0-9_-]*$/.test(agent)) {
    try {
      const promptPath = path.join(PROJECT_ROOT, 'agent-job', 'prompts', `SYSTEM_PROMPT.${agent}.md`);
      if (fs.existsSync(promptPath)) {
        const promptText = fs.readFileSync(promptPath, 'utf8');
        finalDescription = `[Role: ${agent}]\n\n${promptText}\n\n---\n\n# User request\n\n${description}`;
        console.log(`[agent-job] prepended role prompt for agent '${agent}' (${promptText.length} bytes) to description`);
      }
    } catch (e) {
      console.error(`[agent-job] failed to read role prompt for agent '${agent}':`, e.message);
    }
  }
  ...
  const env = [
    ...
    `AGENT_JOB_DESCRIPTION=${finalDescription}`,
  ];
