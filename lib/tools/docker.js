 ...
 */
async function runAgentJobContainer({ agentJobId, repo, branch, title, description, codingAgent, llmModel, planId }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const image = resolveAgentImage(agent);
  ...
  // Note: role-specialist system prompt is NOT injected here. See Patch 11
  // in create-agent-job.js, which commits the role prompt to the branch
  // tree as `agent-job/SYSTEM.md`. The upstream build-system-prompt.sh
  // reads that file from the workspace and assembles SYSTEM_PROMPT
  // naturally. Prior approaches (Patch 8 env push, Patch 10 description
  // prepend) failed because the upstream script blanks SYSTEM_PROMPT mid-
  // setup and because shell expansion in 5_build-prompt.sh broke prompts
  // containing backticks. See [[dockerfile-bundle-lesson]] for the
  // diagnostic story.
  ...
  const env = [
    ...
    // Patch 12 (task 35): expose the role name to the container so
    // `8_create-pr.sh` can skip PR creation for read-only roles
    // (triage / docs / review). Marker: patch12-agent-backend-env.
    `AGENT_BACKEND=${agent}`,
  ];

  // Patch 13 (task 36b): surface the parent plan id to the container so
  // future step scripts (36c fan-out handler) can associate this agent-job
  // with its parent plan row. Marker: patch13-plan-spec-injection.
  if (planId) {
    env.push(`PLAN_ID=${planId}`);
  }
